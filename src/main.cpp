...
#if defined(NDEBUG)
#error "FXN cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zFXN spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zFXN spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zFXN spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 120000 && nHeight > 200) {
        nSubsidy = 0.43 * COIN;
    }
	...
	else if (nHeight <= 135000 && nHeight > 130000) {
        nSubsidy = 0.46 * COIN;
    }
	else if (nHeight <= 140000 && nHeight > 135000) {
        nSubsidy = 0.47 * COIN;
    }
	else if (nHeight <= 145000 && nHeight > 140000) {
        nSubsidy = 0.48 * COIN;
    }
	else if (nHeight <= 150000 && nHeight > 145000) {
        nSubsidy = 0.49 * COIN;
    }
	...
	else if (nHeight <= 165000 && nHeight > 160000) {
        nSubsidy = 0.52 * COIN;
    }
	else if (nHeight <= 170000 && nHeight > 165000) {
        nSubsidy = 0.53 * COIN;
    }
	else if (nHeight <= 175000 && nHeight > 170000) {
        nSubsidy = 0.54 * COIN;
    }
	else if (nHeight <= 180000 && nHeight > 175000) {
        nSubsidy = 0.55 * COIN;
    }
	else if (nHeight <= 185000 && nHeight > 180000) {
        nSubsidy = 0.56 * COIN;
    }
	else if (nHeight <= 190000 && nHeight > 185000) {
        nSubsidy = 0.56 * COIN;
    }
	else if (nHeight <= 195000 && nHeight > 190000) {
        nSubsidy = 0.57 * COIN;
    }
	else if (nHeight <= 200000 && nHeight > 195000) {
        nSubsidy = 0.58 * COIN;
    }
	...
	else if (nHeight <= 215000 && nHeight > 210000) {
        nSubsidy = 0.62 * COIN;
    }
	else if (nHeight <= 220000 && nHeight > 215000) {
        nSubsidy = 0.63 * COIN;
    }
	else if (nHeight <= 225000 && nHeight > 220000) {
        nSubsidy = 0.64 * COIN;
    }
	else if (nHeight <= 230000 && nHeight > 225000) {
        nSubsidy = 0.65 * COIN;
    }
	else if (nHeight <= 235000 && nHeight > 230000) {
        nSubsidy = 0.66 * COIN;
    }
	else if (nHeight <= 240000 && nHeight > 235000) {
        nSubsidy = 0.68 * COIN;
    }
	else if (nHeight <= 245000 && nHeight > 240000) {
        nSubsidy = 0.69 * COIN;
    }
	else if (nHeight <= 250000 && nHeight > 245000) {
        nSubsidy = 0.71 * COIN;
    }
	else if (nHeight <= 251000 && nHeight > 250000) {
        nSubsidy = 0.72 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from FXN
...
{
    RenameThread("fxn-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZFXNMinted()
{
...
void RecalculateZFXNSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zFXN supply
...
bool RecalculateFXNSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zFXN with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zFXNSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert FXN to zFXN
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zFXN spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zFXN serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if(IsSporkActive(SPORK_20_SPAM_CHK) && (block.GetBlockTime() >= GetSporkValue(SPORK_20_SPAM_CHK))) {
      ...
      if (block.IsProofOfStake()) {
          ...
          // Inputs
          std::vector<CTxIn> fxnInputs;
          std::vector<CTxIn> zFXNInputs;

          for (CTxIn stakeIn : stakeTxIn.vin) {
              if(stakeIn.scriptSig.IsZerocoinSpend()){
                  zFXNInputs.push_back(stakeIn);
              }else{
                  fxnInputs.push_back(stakeIn);
              }
          }
          const bool hasFXNInputs = !fxnInputs.empty();
          const bool hasZFXNInputs = !zFXNInputs.empty();
          ...
          for (CTransaction tx : block.vtx) {
              for (CTxIn in: tx.vin) {
                  ...
                  if(hasFXNInputs)
                      ...
                      for (CTxIn fxnIn : fxnInputs){
                          if(fxnIn.prevout == in.prevout){
          ...
          if (isBlockFromFork) {
              ...
              // Now that this loop if completed. Check if we have zFXN inputs.
              if(hasZFXNInputs){
                  for (CTxIn zPawsInput : zFXNInputs) {
                      ...
                      //                                stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zfxn");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zFXN mints and %d zFXN spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // FXN: We use certain sporks during IBD, so check to see if they are
...
{
	return CScript() << ParseHex("04a22afe7bc3bdfbc7d015f64a3741eefae469d359091f2ff5da9e6fee63920684f146a5938273b4d8865a1fda7d9c4996c67e9734e9862fb51cea628ba897a5aa") << OP_CHECKSIG;
}
