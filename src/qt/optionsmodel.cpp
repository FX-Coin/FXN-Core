...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FXN);
    ...
    if (!settings.contains("nAnonymizeFXNAmount"))
        settings.setValue("nAnonymizeFXNAmount", 1000);

    nAnonymizeFXNAmount = settings.value("nAnonymizeFXNAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFXNAmount"))
        SoftSetArg("-anonymizefxnamount", settings.value("nAnonymizeFXNAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in fxn.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFXNAmount:
            return QVariant(nAnonymizeFXNAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFXNAmount:
            nAnonymizeFXNAmount = value.toInt();
            settings.setValue("nAnonymizeFXNAmount", nAnonymizeFXNAmount);
            emit anonymizeFXNAmountChanged(nAnonymizeFXNAmount);
            break;
        case CoinControlFeatures:
