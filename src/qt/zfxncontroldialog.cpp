...
// Copyright (c) 2017-2018 The FXN developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zfxncontroldialog.h"
#include "ui_zfxncontroldialog.h"
...
std::list<std::string> ZFXNControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZFXNControlDialog::listMints;

ZFXNControlDialog::ZFXNControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZFXNControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZFXNControlDialog::~ZFXNControlDialog()
{
    delete ui;
}

void ZFXNControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFXNControlDialog::updateList()
{
...
void ZFXNControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFXNControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZFXN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFXNControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZFXNControlDialog::GetSelectedMints()
{
...
void ZFXNControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
